...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		while(!queue.isEmpty()&&!visited.contains(word2)) {
			...
			List<String> list = nw.distanceOne(curr.getWord(), true);
			for(String w : list) {
				if(!visited.contains(w)) {
					curr.addChild(w);
					visited.add(w);
					queue.add(curr.addChild(w));
					if(w.equals(word2)) {
...
class WPTreeNode {
    ...
    *
    ...
    public void setParent(WPTreeNode node) {
    	parent = node;
    }
    public void setChildren(LinkedList<WPTreeNode> list) {
    	children = list;
    }
